...
最初は入口と標準automation memoryだけを読む。手順は該当する一つだけ開く。共通規則は再記載しない。
...
日次は調査・ローカルの差分準備まで。commit・push・PR作成はそのターンの明示承認が必要。最終マージは利用者が行い、Codexはmerge・auto-merge設定・手動公開を行わない。Memoryは索引であり承認ではない。
