...
<template>
    ...
    <header class="sticky top-0 z-50 backdrop-blur
        ...
        <div class="max-w-6xl mx-auto px-4 flex items-center justify-between h-14">
            ...
            <button class="md:hidden p-2 rounded
               ...
               focus-visible:ring-2 focus-visible:ring-primary-500" @click="toggle" aria-label="Toggle main menu"
                ...
                <svg v-if="!open" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24"
                    stroke="currentColor" fill="none">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg"
                    class="h-6 w-6 rotate-180 transition-transform duration-300" viewBox="0 0 24 24"
                    stroke="currentColor" fill="none">
            ...
            <nav class="hidden md:flex space-x-6" aria-label="Main navigation">
                <a v-for="l in links" :key="l.to" :href="l.to" class="text-gray-700 dark:text-on-surface
                  hover:text-primary-600 dark:hover:text-primary-300 transition">
                    {{ l.label }}
                </a>
            </nav>
        </div>
        ...
        <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 -translate-y-2"
            ...
            <nav v-if="open" id="mobile-nav" aria-label="Mobile navigation" class="md:hidden bg-white dark:bg-surface
               border-t border-gray-100 dark:border-surface-muted">
